...
def expulse_user():
    ...
    while True:
        ...
        for user in collection1.find({"dias": {"$lt": time.time()}}):
            ...
            url = f'https://api.telegram.org/bot6956382268:AAGp_b7FYHJaDoSYyITPL72i5FDZ97iDF58/sendMessage'
            params = {'chat_id': user['id'],'text': '<b>Se acabo tu acceso al grupo de usar nuestro bot.❗️</b>','parse_mode': 'HTML'}
            
            requests.post(url=url, params=params)

            url = f'https://api.telegram.org/bot6956382268:AAGp_b7FYHJaDoSYyITPL72i5FDZ97iDF58/sendMessage'
            params = {'chat_id': -4286695568,'text': f'<b>Se le acabo el acceso de dias al chat con id :( {user["id"]}  )❗️</b>','parse_mode': 'HTML'}
        ...
        for user in collection.find({"since": {"$lt": time.time()}}):
            ...
            url = f'https://api.telegram.org/bot6956382268:AAGp_b7FYHJaDoSYyITPL72i5FDZ97iDF58/sendMessage'
            params = {'chat_id': -4286695568,'text': f'<b>Se le acabo el plan premium al usuario( {user["id"]}  )❗️</b>','parse_mode': 'HTML'}
